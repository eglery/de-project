CREATE MATERIALIZED VIEW IF NOT EXISTS author_h_index AS
SELECT
    ad.id AS author_id,
    ad.author,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS journal_h_index AS
SELECT
    jd.id AS journal_id,
    jd.container_title,
    MAX(sub.rn) AS h_index
...
CREATE MATERIALIZED VIEW IF NOT EXISTS category_h_index AS
SELECT
...
CREATE MATERIALIZED VIEW IF NOT EXISTS publisher_yearly_rank AS
